...
| Data assertions hardcoded in the render layer | Alerts/banners must be rules over data fields, never literal ticker+number claims baked into the template JS. A hardcoded "was up +$902" froze into a stale lie the day after it was written. If a number can change, it belongs in the payload and gets computed at render time |
| Relative-time words in persisted text | "today"/"this week" in any text that survives a rebuild becomes a lie tomorrow. Use absolute dates ("7/6 gap-up"). Same principle: notes carry decision rationale only — live numbers (PnL, price) belong in fields that refresh |
